...
- `internal/monitor/model.go` — add the fields to both `DesiredExternalMonitor` and `ActualExternalMonitor`, plus a `HeaderField` type local to the package. `internal/monitor` does not import `api/v1alpha1` today, and keeping that direction of dependency absent means the intermediate model stays independent of the CRD version. `internal/source` translates between the two. The desired struct keeps `omitempty` on every new field, so a field left at its zero value does not change the hash — with the single exception of `maxCheckAttempts`, which the source layer normalises to `1` and which therefore does serialise. See Backward Compatibility.
